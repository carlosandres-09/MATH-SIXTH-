...
import os
import tkinter as tk  # Importación correcta de tkinter
...
# Cargar y procesar múltiples audios
def cargar_y_procesar_varios():
    ...
    for file in files:
        ...
        # Filtrado de frecuencias alrededor de la frecuencia fundamental
        ...
        # Tono puro basado en frecuencia fundamental
        ...
        ultimo_tono = tone_pure  # Guardamos el tono para reproducir al cierre

        # Reproducir tono puro
        sd.play(tone_pure, samplerate=fs)
        sd.wait()

        # Filtro pasa-bajos en señal original
        cutoff = 300
...
# Reproducir el último tono al cerrar la ventana
def on_closing():
...
# Interfaz gráfica
root = tk.Tk()  # Creación de la ventana raíz con tkinter
root.title("Procesador de Audio")

# Crear el label de los creadores
label_creadores = tk.Label(root, text="By Carlos Soacha, Johan Leal, Jhordy Miranda", font=("Arial", 12))
label_creadores.pack(pady=10)

# Botón para cargar y procesar audios
button = tk.Button(root, text="Cargar y Procesar Varios Audios", command=cargar_y_procesar_varios, 
...
# Configuración para reproducir el último tono al cerrar
